...
function bfnd_render_home() {
    ...
    $hero_image = wp_get_attachment_image_url((int) get_option('bfnd_home_hero_id'), 'full') ?: bfnd_asset('assets/brand/home-editorial.png');
    ...
    $school_promo = wp_get_attachment_image_url((int) get_option('bfnd_school_hero_id'), 'large') ?: bfnd_asset('assets/brand/school-editorial.png');
...
function bfnd_render_lifestyle() {
    $hero = wp_get_attachment_image_url((int) get_option('bfnd_lifestyle_hero_id'), 'full') ?: bfnd_asset('assets/brand/lifestyle-editorial.png');
    ...
    echo '<section class="bf-lifestyle-intro-grid"><div class="bf-lifestyle-intro-copy"><span class="bf-kicker">A LITTLE WOOD, A BETTER DAY</span><h2>生活木作的<br>可能性</h2><p>木作不只存在於大件家具，也能出現在餐桌、書桌、廚房、孩子的房間，甚至是旅途中的風景。我們持續探索木頭在生活中的各種可能，用手作的溫度，連結人與物、日常與自然。</p></div><div class="bf-lifestyle-intro-image">'; bfnd_image(bfnd_asset('assets/brand/wooden-cup-editorial.png'), '木杯與木托盤・生活木作情境示意圖'); echo '<small class="bf-editorial-caption">木杯情境示意，非正式販售作品</small></div></section>';
    ...
    foreach ($steps as $step) { $photo = wp_get_attachment_image_url((int) get_option($step[2]), 'large') ?: bfnd_asset($step[3]); echo '<article><div>'; bfnd_image($photo, $step[0] . '・木作情境示意圖'); echo '</div><h3>' . bfnd_e($step[0]) . '</h3><p>' . bfnd_e($step[1]) . '</p></article>'; }
...
function bfnd_render_school() {
    $hero = wp_get_attachment_image_url((int) get_option('bfnd_school_hero_id'), 'full') ?: bfnd_asset('assets/brand/school-editorial.png');
    ...
    if (!$online) {
        ...
        foreach ($previews as $preview) {
            echo '<article class="bf-course-card bf-course-preview"><div class="bf-course-image">'; bfnd_image(bfnd_asset($preview[2]), $preview[0] . '課程情境示意圖'); echo '</div><div class="bf-course-card-body"><span class="bf-kicker">ONLINE COURSE / 籌備中</span><h3>' . bfnd_e($preview[0]) . '</h3><p>' . bfnd_e($preview[1]) . '</p><span class="bf-preview-status">尚未開放報名</span></div></article>';
        }
    ...
    echo '</div></section>';
    $school_cta = wp_get_attachment_image_url((int) get_option('bfnd_home_hero_id'), 'full') ?: bfnd_asset('assets/brand/home-editorial.png');
...
function bfnd_render_story() {
    $brand_hero = wp_get_attachment_image_url((int) get_option('bfnd_home_hero_id'), 'full') ?: bfnd_asset('assets/brand/home-editorial.png');
    ...
    $wood_ring = wp_get_attachment_image_url((int) get_option('bfnd_manifesto_id'), 'large') ?: bfnd_asset('assets/brand/wood-ring-editorial.png');
    ...
    $workshop = wp_get_attachment_image_url((int) get_option('bfnd_school_hero_id'), 'large') ?: bfnd_asset('assets/brand/school-editorial.png');
    ...
    $cnc_story = wp_get_attachment_image_url((int) get_option('bfnd_process_cnc_id'), 'large') ?: bfnd_asset('assets/courses/editorial-cnc.png');
...
function bfnd_render_collaboration() {
    $hero = wp_get_attachment_image_url((int) get_option('bfnd_home_hero_id'), 'full') ?: bfnd_asset('assets/brand/home-editorial.png');
    ...
    foreach ($scenarios as $scene) { $photo = wp_get_attachment_image_url((int) get_option($scene[2]), 'large') ?: bfnd_asset($scene[3]); echo '<article><div>'; bfnd_image($photo, $scene[0] . '情境示意圖'); echo '</div><h3>' . bfnd_e($scene[0]) . '</h3><p>' . bfnd_e($scene[1]) . '</p></article>'; }
    echo '</div></div></section>';
    $forest = wp_get_attachment_image_url((int) get_option('bfnd_forest_id'), 'full') ?: bfnd_asset('assets/brand/forest-editorial.png');
    ...
    $after_photo = wp_get_attachment_image_url((int) get_option('bfnd_process_sanding_id'), 'large') ?: bfnd_asset('assets/courses/editorial-beginner.png');
